...
namespace LazyCache.Providers.FilesCaches
{
  ...
  public class MemoryCache : IMemoryCache, IDisposable
  {
    ...
    {
      _options = optionsAccessor != null ? optionsAccessor.Value : throw new ArgumentNullException(nameof (optionsAccessor));
      _entries = new ConcurrentDictionary<object, CacheEntry>();
      _setEntry = new Action<CacheEntry>(SetEntry);
      _entryExpirationNotification = new Action<CacheEntry>(EntryExpired);
      if (_options.Clock == null)
        _options.Clock = (ISystemClock) new SystemClock();
    ...
    {
      get => (ICollection<KeyValuePair<object, CacheEntry>>) _entries;
    }
    ...
    public ICacheEntry CreateEntry(object key)
    {
      ...
      return (ICacheEntry) new CacheEntry(key, _setEntry, _entryExpirationNotification);
    }
    ...
    private void SetEntry(CacheEntry entry)
    {
      ...
      if (nullable1.HasValue)
      {
        ...
        if (!nullable1.HasValue)
          throw new InvalidOperationException(string.Format("Cache entry must specify a value for {0} when {1} is set.", (object) "Size", (object) "SizeLimit"));
      }
      ...
      if (entry._absoluteExpirationRelativeToNow.HasValue)
      {
        ...
        nullable2 = expirationRelativeToNow.HasValue ? new DateTimeOffset?(dateTimeOffset + expirationRelativeToNow.GetValueOrDefault()) : new DateTimeOffset?();
      }
      else if (entry._absoluteExpiration.HasValue)
        nullable2 = entry._absoluteExpiration;
      if (nullable2.HasValue && (!entry._absoluteExpiration.HasValue || nullable2.Value < entry._absoluteExpiration.Value))
    ...
    private void StartScanForExpiredItems()
    {
      ...
      Task.Factory.StartNew((Action<object>) (state => ScanForExpiredItems((MemoryCache) state)), (object) this, CancellationToken.None, TaskCreationOptions.DenyChildAttach, TaskScheduler.Default);
    }

    private static void ScanForExpiredItems(MemoryCache cache)
    {
      DateTimeOffset utcNow = cache._options.Clock.UtcNow;
      foreach (CacheEntry entry in (IEnumerable<CacheEntry>) cache._entries.Values)
      {
    ...
    {
      ThreadPool.QueueUserWorkItem((WaitCallback) (s => OvercapacityCompaction((MemoryCache) s)), (object) this);
    }
    ...
    private static void OvercapacityCompaction(MemoryCache cache)
    {
      ...
      double? nullable1 = sizeLimit.HasValue ? new double?((double) sizeLimit.GetValueOrDefault()) : new double?();
    ...
    {
      Compact((long) (int) ((double) _entries.Count * percentage), (Func<CacheEntry, long>) (_ => 1L));
    }
    ...
    private void Compact(long removalSizeTarget, Func<CacheEntry, long> computeEntrySize)
    {
      ...
      long removedSize = 0;
      DateTimeOffset utcNow = _options.Clock.UtcNow;
      foreach (CacheEntry cacheEntry in (IEnumerable<CacheEntry>) _entries.Values)
      {
      ...
      ExpirePriorityBucket(ref removedSize, removalSizeTarget, computeEntrySize, entriesToRemove, priorityEntries1);
      ExpirePriorityBucket(ref removedSize, removalSizeTarget, computeEntrySize, entriesToRemove, priorityEntries2);
      ExpirePriorityBucket(ref removedSize, removalSizeTarget, computeEntrySize, entriesToRemove, priorityEntries3);
      ...
      List<CacheEntry> priorityEntries)
    {
      if (removalSizeTarget <= removedSize)
        return;
      foreach (CacheEntry cacheEntry in (IEnumerable<CacheEntry>) Enumerable.OrderBy<CacheEntry, DateTimeOffset>((IEnumerable<CacheEntry>) priorityEntries, (Func<CacheEntry, DateTimeOffset>) (entry => entry.LastAccessed)))
      {
