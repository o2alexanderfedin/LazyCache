...
public class MemoryCacheBenchmarks
{
    ...
    public ComplexObject DotNetMemoryCache_GetOrAdd_Hit() =>
        PopulatedMemCache.GetOrCreate(CacheKey, entry => ComplexObject);
    ...
    public Task<ComplexObject> DotNetMemoryCache_GetOrAddAsync_Miss() =>
        MemCache.GetOrCreateAsync(CacheKey, entry => Task.FromResult(ComplexObject));

    [Benchmark, BenchmarkCategory(nameof(IAppCache.GetOrAddAsync) + "_Miss")]
    public Task<ComplexObject> LazyCache_GetOrAddAsync_Miss() =>
        AppCache.GetOrAddAsync(CacheKey, entry => Task.FromResult(ComplexObject));
    ...
    public Task<ComplexObject> DotNetMemoryCache_GetOrAddAsync_Hit() =>
        PopulatedMemCache.GetOrCreateAsync(CacheKey, entry => Task.FromResult(ComplexObject));

    [Benchmark, BenchmarkCategory(nameof(IAppCache.GetOrAddAsync) + "_Hit")]
    public Task<ComplexObject> LazyCache_GetOrAddAsync_Hit() =>
        PopulatedAppCache.GetOrAddAsync(CacheKey, entry => Task.FromResult(ComplexObject));
}